...
### Активные проблемы
1. **Неполный backprop** — обновляются только `lm_head` и `tok_emb`.
   Attention и FF веса не обучаются. Исправление: Фаза 2 (ROADMAP).

2. **SGD оптимизатор** — простой SGD нестабилен при обучении.
   Нужен Adam. Исправление: Фаза 2 (ROADMAP).

3. **Мало русских данных** — 18KB недостаточно для хорошего русского.
   Нужно: 200KB+ качественных русских текстов.

4. **Нет KV-cache** — каждый токен пересчитывает всю историю.
...
2. **Следующий приоритет** (Фаза 2 из ROADMAP):
   - Реализовать полный backprop через FF слои
   - Добавить Adam optimizer
   - Добавить больше русских обучающих данных (200KB+)
...
*Создан: июль 2026*
*Последнее обновление: июль 2026 (v2.0)*
