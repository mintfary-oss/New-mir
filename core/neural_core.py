...
def _forward_and_cache(
    ...
    layer_caches : list[dict]
        One dict per layer with keys for both attention and FF backprop:

        Attention (for w_q/k/v/o gradients):
          ``"x_norm_attn"``  (T, D)  — LN1 output (input to Q/K/V projections)
          ``"q_h"``          (H, T, Dh) — query heads
          ``"k_h"``          (H, T, Dh) — key heads
          ``"v_h"``          (H, T, Dh) — value heads
          ``"attn"``         (H, T, T)  — softmax attention weights
          ``"ctx"``          (T, D)     — merged context before w_o

        Feed-forward (for w1/b1/w2/b2 gradients):
          ``"ff_input"``     (T, D)  — LN2 output (input to ff_w1)
          ``"ff_pre"``       (T, FF) — pre-GELU hidden states
    """
    t = len(tokens)
    if t == 0:
        return np.zeros((0, weights.embed_dim), dtype=np.float32), []
    ...
    for layer in range(weights.num_layers):
        ...
        # --- Attention sub-layer (inlined to capture intermediate values) ---
        ...
        ff_pre = ff_input @ weights.ff_w1[layer] + weights.ff_b1[layer]
        ff_out = _gelu(ff_pre) @ weights.ff_w2[layer] + weights.ff_b2[layer]
        x = x + ff_out
...
def _backward_attn_layer(
    ...
    q_h    = cache["q_h"]           # (H, T, Dh)
    k_h    = cache["k_h"]
    v_h    = cache["v_h"]
    attn   = cache["attn"]          # (H, T, T) — softmax weights
    ctx    = cache["ctx"]           # (T, D)
    ...
    # ctx_h = attn @ v_h  →  d_v_h = attn.T @ d_ctx_h
    d_v_h = attn.transpose(0, 2, 1) @ d_ctx_h                        # (H, T, Dh)

    # d_attn_h = d_ctx_h @ v_h.T  (H, T, T)
    d_attn = d_ctx_h @ v_h.transpose(0, 2, 1)                        # (H, T, T)

    # 3. Softmax backward: d_scores = attn*(d_attn - sum(d_attn*attn, -1))
    ...
    # 4. Gradients for q, k, v heads
    d_q_h = d_scores @ k_h                                            # (H, T, Dh)
    d_k_h = d_scores.transpose(0, 2, 1) @ q_h                        # (H, T, Dh)
    ...
    grad_wq = x_norm.T @ d_q                                          # (D, D)
    grad_wk = x_norm.T @ d_k
    grad_wv = x_norm.T @ d_v

    adam.update(f"attn_wq_{layer}", weights.attn_wq[layer], grad_wq)
    adam.update(f"attn_wk_{layer}", weights.attn_wk[layer], grad_wk)
    adam.update(f"attn_wv_{layer}", weights.attn_wv[layer], grad_wv)

    # 6. Gradient at x_norm (through q/k/v projections)
    d_x_norm = (d_q @ weights.attn_wq[layer].T
                + d_k @ weights.attn_wk[layer].T
                + d_v @ weights.attn_wv[layer].T)                     # (T, D)

    # Return gradient at x_in:
    #   residual connection + attention branch (LN Jacobian ≈ identity)
    return d_x + d_x_norm
        ...
        * **tok_emb gradient** — embedding layer is still updated with a
        ...
        for epoch in range(epochs):
            ...
            for text in examples:
                ...
                # 2. Full backward through all layers (reversed — deepest first)
                for layer in reversed(range(w.num_layers)):
                    cache = layer_caches[layer]

                    # --- FF backward ---
                    ff_input = cache["ff_input"]   # (T-1, D)
                    ff_pre   = cache["ff_pre"]     # (T-1, FF)
                    ff_h     = _gelu(ff_pre)       # (T-1, FF)
                    ...
                    d_ff_h   = d_x @ w.ff_w2[layer].T
                    d_ff_pre = d_ff_h * _gelu_derivative(ff_pre)

                    grad_w1 = ff_input.T @ d_ff_pre
                    grad_b1 = d_ff_pre.sum(axis=0)
                    self._adam.update(f"ff_w1_{layer}", w.ff_w1[layer], grad_w1)
                    self._adam.update(f"ff_b1_{layer}", w.ff_b1[layer], grad_b1)

                    # FF residual — gradient now at x_mid (after attn)
                    d_x = d_x + d_ff_pre @ w.ff_w1[layer].T

                    # --- Attention backward ---
                    # d_x is gradient at x_mid; returns gradient at x_in
